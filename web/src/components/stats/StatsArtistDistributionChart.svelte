    ...
    {:else}
      <div class="distribution-legend" aria-label="Artist distribution legend">
        {#each entities as entity, index (entity.id)}
          ...
          {:else}
            <a href={artistTransitionHref(entity.id, `stats-distribution-${index}`)} style={`--swatch: ${distributionColor(index, entity)};`} title={`${entity.name}: ${formatDistributionValue(entity.total)}`}>
